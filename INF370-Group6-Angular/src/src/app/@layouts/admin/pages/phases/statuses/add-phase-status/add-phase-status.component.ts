...
import { AddProjectStatusComponent } from '../../../projects/project-statuses/add-project-status/add-project-status.component';

@Component({
  selector: 'app-add-phase-status',
  templateUrl: './add-phase-status.component.html',
  styleUrls: ['./add-phase-status.component.scss']
})
export class AddPhaseStatusComponent implements OnInit {
  errorMessage = "";
  ...
  onSubmit() {
    ...
    if (this.addFrom.valid) {
      this._phaseService.addPhasesStatus(this.addFrom.value, this._authService.currentUser.UserName)
        .subscribe(event => {
